...
hw.module @Foo (in %x: i32, out y: i32) {
  ...
  // CHECK-NEXT:   [[V1:%.+]] = systemc.cpp.new([[NAME0]]) : (!emitc.ptr<!emitc.opaque<"const char">>) -> !emitc.ptr<!emitc.opaque<"VBar">>
  ...
  // CHECK-NEXT:   [[V6:%.+]] = systemc.cpp.new([[NAME1]]) : (!emitc.ptr<!emitc.opaque<"const char">>) -> !emitc.ptr<!emitc.opaque<"VEmpty">>
